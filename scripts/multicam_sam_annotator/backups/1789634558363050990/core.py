...
class Project:
 ...
 def reassign_box(self,f,c,obj,new_id,class_id):
  ...
  mask=self.mask(e).copy()
  if new_id==obj:
   self.state['objects'][obj]['class_id']=class_id
   self.put(f,c,obj,mask,'manual',True);affected=self.invalidate(f,obj);self.save();self.refresh_exports(set([int(i) for i in self.state['frames']]+affected));return obj
  self.delete_box(f,c,obj)
  return self.add(dict(frame=f,cam=c,object=new_id,class_id=class_id,mask=encode(mask)))
 def stop(self,f,obj):
  obj=str(obj)
  if obj not in self.state['objects']:raise ValueError('unknown object')
