...
from core import Project,Segmenter,encode,png,CLASSES,smart_segment
import cv2
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  try:
   ...
   d=json.loads(self.rfile.read(size));f=int(d.get('frame',0));c=int(d.get('cam',0));log=[];saved_object=None
   if self.path=='/smart':
    m,score,box=smart_segment(project.seg,project.image(f,c),d['box']);self.send(dict(mask=encode(m),score=score,box=box));return
   elif self.path=='/sam':
    ...
    obj=project.add(d);saved_object=obj
   ...
   elif self.path=='/delete_box':project.delete_box(f,c,str(d['object']))
   elif self.path=='/reassign_box':saved_object=project.reassign_box(f,c,str(d['object']),str(d.get('new_id','')),int(d['class_id']))
   ...
   self.send(dict(data=project.data(f),log=log,saved_object=saved_object))
